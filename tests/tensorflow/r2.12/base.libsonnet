          ...
          'create-tpu': {
            ...
            command: utils.scriptCommand(|||
              ...
              if [[ ${softwareVersion: -3} == "pod" ]]; then
                 gcloud alpha compute tpus tpu-vm ssh  ${tpu_name} --project=${project} --zone=${zone} --worker=all --command="sudo systemctl daemon-reload && sudo systemctl restart tpu-runtime"
              fi
              sleep %(sleepTime)d
            ||| % tpuCreateSettings),
            env: [
